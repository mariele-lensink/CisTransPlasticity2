...
#downloaded gene names for probe ids from tair, this is a conversion table
probedf <- data.table(read.delim("~/Documents-local/SalicylicAcid/probeidconversiontabble.tsv"))
probedf<-probedf[,1:2]
#probe id gene name conversion list from Dan
probedf_full <-data.table(read.xlsx("Row Labels in Order.xlsx"))
...
gmap<-fread("delta/BayxSha_gmap.csv")
...
fwrite(peaksfinal,"finalpeaks_withmarkernames_10cMwindow_LODinfo.txt")
